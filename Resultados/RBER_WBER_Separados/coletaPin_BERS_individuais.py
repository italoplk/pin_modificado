...
for video in videos:
	...
	arquivoEscrita = open("./leitura_Pin_BER_individual/"+str(video) + ".csv", "w")
	
	arquivoEscrita.write(" , RBER 1E-3, RBER 1E-3, RBER 1E-4, RBER 1E-4, RBER 1E-5, RBER 1E-5, RBER 1E-6, RBER 1E-6, RBER 1E-7, RBER 1E-7" )
	for qp in QPs:
		arquivoEscrita.write("\n QP " + str(qp) + ",")
	
		for rber in BERS:
			arquivoLeitura = open("./" + str(video) + "/csv/" + str(video) + "_qp"+str(qp)+ "_RBER_"+str(rber) + ".csv", "r")
...
for video in videos:
	...
	arquivoEscrita.write("\n\n , WBER 1E-3, WBER 1E-3, WBER 1E-4, WBER 1E-4, WBER 1E-5, WBER 1E-5, WBER 1E-6, WBER 1E-6, WBER 1E-7, WBER 1E-7" )
	for qp in QPs:
		arquivoEscrita.write("\n QP " + str(qp) + ",")
	
		for wber in WBERS:
			arquivoLeitura = open("./" + str(video) + "/csv/" + str(video) + "_qp"+str(qp)+ "_WBER_"+str(wber) + ".csv", "r")
			for line in arquivoLeitura:
				split = line.split(",")
				if split[0] == "Command":
					linha = arquivoLeitura.next()

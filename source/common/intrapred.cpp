...
void intra_pred_ang_c(pixel* dst, intptr_t dstStride, const pixel *srcPix0, int dirMode, int bFilter)
{
    ...
    //@IDM buffer de vizinhos. srcPix eh candidato
    ...
    if (!angle)
    {
        for (int y = 0; y < width; y++)
            for (int x = 0; x < width; x++)
                dst[y * dstStride + x] = srcPix[1 + x]; //@IDM acho que dst seja a referencia do modo vertical (25)
    ...
    else // Angular prediction.
    {
        ...
        if (angle < 0)
        {
            ...
            for (int i = 0; i < nbProjected; i++)
            {
                ...
                ref_pix[- 2 - i] = srcPix[width2 + (invAngleSum >> 8)]; //@IDM maybe this one
            }

            // Copy the top-left and top pixels. @IDM top pixels ref_pix seria o que queremos probably...
            for (int i = 0; i < width + 1; i++)
